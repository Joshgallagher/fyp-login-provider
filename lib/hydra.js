...
const get = async (flow, challenge) => {
    const baseEndpoint = new URL(`/oauth2/auth/requests/${flow}`, hydraAdminUrl)
    baseEndpoint.search = queryString.stringify({ [flow + '_challenge']: challenge })

    let { response, body } = await fetch(baseEndpoint.toString)

    // TODO: Handle errors - refer to node consent example
...
const put = async (flow, action, challenge, body) => {
    const baseEndpoint = new URL(`/oauth2/auth/requests/${flow}/${action}`, hydraAdminUrl)
    baseEndpoint.search = queryString.stringify({ [flow + '_challenge']: challenge })

    let { response, body } = await fetch(
